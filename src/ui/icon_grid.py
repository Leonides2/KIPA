...
from src.ui.qt_compat import QtCore, QtGui, QtWidgets, Qt, Signal, exec_dialog
...
class IconEditDialog(QtWidgets.QDialog):
    """Diálogo de edición de tamaños/contexto para un icono individual."""

    def __init__(self, icon: IconItem, parent: QtWidgets.QWidget | None = None):
        ...
        self.name_edit = QtWidgets.QLineEdit(icon.name)
...
class IconGridWidget(QtWidgets.QWidget):
    ...
    def __init__(self, icon_manager: IconManager, parent: QtWidgets.QWidget | None = None):
        ...
        self.add_button = QtWidgets.QPushButton("Añadir iconos…")
        self.add_button.clicked.connect(self._on_add_clicked)
        toolbar.addWidget(self.add_button)

        self.remove_button = QtWidgets.QPushButton("Eliminar seleccionado")
        ...
        self.table.horizontalHeader().setSectionResizeMode(
            COL_NAME, QtWidgets.QHeaderView.Stretch
        )
    ...
    def _add_paths(self, paths: list[Path]) -> None:
        ...
        if errors:
            QtWidgets.QMessageBox.warning(
                self, "Algunos iconos no se añadieron", "\n".join(errors)
            )
        if paths:
            self.refresh()
    ...
    def _on_replace_clicked(self) -> None:
        ...
        try:
            ...
            QtWidgets.QMessageBox.warning(self, "No se pudo reemplazar", str(exc))
            return
        self.refresh()
        self.icons_changed.emit()
    ...
    def _append_row(self, icon: IconItem) -> None:
        ...
        qicon = QtGui.QIcon(str(icon.source_path)) if icon.source_path.exists() else QtGui.QIcon()
        preview_item.setIcon(qicon)
        preview_item.setFlags(preview_item.flags() & ~Qt.ItemIsEditable)
        self.table.setItem(row, COL_PREVIEW, preview_item)

        name_item = QtWidgets.QTableWidgetItem(icon.name)
        name_item.setData(Qt.UserRole, icon.name)
        self.table.setItem(row, COL_NAME, name_item)

        context_item = QtWidgets.QTableWidgetItem(
            CONTEXT_DISPLAY_NAMES.get(icon.context, icon.context)
        )
        ...
        sizes_item = QtWidgets.QTableWidgetItem(sizes_label or "(sin configurar)")
        sizes_item.setFlags(sizes_item.flags() & ~Qt.ItemIsEditable)
        self.table.setItem(row, COL_SIZES, sizes_item)

        edit_button = QtWidgets.QPushButton("Configurar…")
